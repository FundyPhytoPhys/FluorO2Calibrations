...
DataIn <- file.path("..", "RawData", "SySl", "Solisense", "SySlCCACaloxyBiolSteps", fsep = .Platform$file.sep)
...
SolFitsTrim <- SolFits %>% 
  ...
  mutate(Filename = str_remove(string = Filename, pattern = "../RawData/SySl/Solisense/SySlCCACaloxyBiolSteps/"),
         Filename = str_remove(string = Filename, pattern = "_caloxy"),
         Filename = str_remove(string = Filename, pattern = "_fit.csv")
         ) %>%
  separate(Filename, into = c("Project", "RunDateTime", "CultureID", "Ex_WL", "GrowthLight"), sep = "([\\/\\_])", remove = FALSE) %>%
...
SolFitsTrim <- SolFitsTrim %>%
  group_by(SourceDataFile, Filename, Project, CultureID, Ex_WL, TempCont) %>%
