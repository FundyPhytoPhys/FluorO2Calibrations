...
DataIn <- file.path("..", "RawData", "SolisenseSySl", Run, fsep = .Platform$file.sep)
...
#Think of better ways to do this
 "../RawData/SolisenseSySl/SySlCCACaloxy/PICO_202205181355_caloxy_SySl1246_535_LL_fit.csv"

SolFitsTrim <- SolFits %>% 
  filter(!grepl("----", DATE)) %>% # remove rows with "----" in DATE
  select(-c("RFID_User_Data", "Barcode_Data", "Lon", "Lat", "GPS_stat", "...58")) %>% # remove superfluous columns
  mutate(Filename = str_remove(string = Filename, pattern = "../RawData/SolisenseSySl/"),
         Filename = str_remove(string = Filename, pattern = "_caloxy"),
         Filename = str_remove(string = Filename, pattern = "_fit"),
         Filename = str_remove(string = Filename, pattern = ".csv")
         ) %>%
  separate(Filename, into = c("Run", "Project", "RunDateTime", "CultureID", "Ex_WL", "GrowthLight"), sep = "([\\/\\_])", remove = FALSE) %>%
  ...
  mutate(FvFm = as.numeric(as.character(FvFm)),
         FvFo = as.numeric(as.character(FvFo)),
...
SolFitsTrim <- SolFitsTrim %>%
  mutate(across(.cols = c(Light_1:QBP_Size), .fns = as.numeric)) %>%
...
SolFitsTrim <- SolFitsTrim %>%
  group_by(Filename, Run, Project, CultureID, Ex_WL) %>%
...
```
Problem with ActPAR for
Ex_WL = 445 & SySl1158, SySl1159, SySl1191, SySl1192 (0 only)
Ex_WL = 505 & SySl1189 (missing all), SySl1191 (0 only)
```{r}
# test <- SolFitsTrim %>%
...
SolFitsTrim2 <- SolFitsTrim %>%
  group_by(Filename, Project, Run, CultureID, Ex_WL, TempCont) %>%
  mutate(Fodark = Fo[1],
         Fmdark = Fm[1],
         Sigdark = Sig[1],
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = aLHIIOxbomax, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = Ctauav, y = qpOxbo, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = JVPSII_ETRtauav_FoSig, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
  theme_bw()
...
SolFitsTrim2 %>%
  ...
  ggplot() +
  geom_point(aes(x = 
         JVPSII_ETRqpOxbo_FoSig, y = JVPSII_ETRtauav_aLHII_Sig, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
  #coord_fixed(ratio = 1) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x =  JVPSII_ETRqpOxbo_FoSig, y = JVPSII_aLHIIOxbomax, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
  #coord_fixed(ratio = 1) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = ETRCtauav, y = ETRGorbo, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
  coord_fixed(ratio = 1) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
