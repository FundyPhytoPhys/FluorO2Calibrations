...
#relies upon phiPSII which is not absolute in cyanos
         JVPSII_ETRC_TauAv_FoSig = ETRC_TauAv * Fomin/Sigmax, #Sigmax not converted A2_m2
         JVPSII_ETRqpOxbo_FoSig = ETRqpOxbo * Fomin/Sigmax,
         JVPSII_ETRC_TauAv_aLHII_Sig = ETRC_TauAv * aLHIIOxbomax/Sigmax,
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = JVPSII_ETRC_TauAv_FoSig, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
  theme_bw()
...
SolFitsTrim2 %>%
  ...
  ggplot() +
  geom_point(aes(x = 
         JVPSII_ETRqpOxbo_FoSig, y = JVPSII_ETRC_TauAv_aLHII_Sig, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
  #coord_fixed(ratio = 1) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
