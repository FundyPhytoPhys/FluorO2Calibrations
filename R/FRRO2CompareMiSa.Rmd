...
O2FRRfMeta %>%
  filter(ActPARCorrinter > 0 & ActPARCorrinter < 200) %>%
  ggplot() +
  geom_point(aes(y  = (O2evln_O2ls * 4)/ETRC_WtTau, x = ActPARCorr, colour = Ex_WL, shape = Strain)) +
  scale_colour_manual(values = Colours_nm) +
  geom_smooth(aes(x = ActPARCorr, y =(O2evln_O2ls * 4)/ETRC_WtTau, colour = Ex_WL), method = "lm") +
  #facet_grid(cols = vars(Strain)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +
  theme_bw()
...
O2FRRfMetaNest <-  O2FRRfMeta %>%
  ...
  JVPSII_ETRC_WtTau_O2 = map(data, possibly(~lm(JVPSII_ETRC_WtTau ~ O2evln_O2ls, data = .x), otherwise = NULL)),
  JVPSII_ETRC_WtTau_O2_tidied =  map(JVPSII_ETRC_WtTau_O2, possibly(tidy, otherwise = NULL)),
  JVPSII_ETRC_WtTau_O2_param = map(JVPSII_ETRC_WtTau_O2, possibly(glance, otherwise = NULL)),
  JVPSII_ETRC_WtTau_O2_predict =  map(JVPSII_ETRC_WtTau_O2, possibly(augment, otherwise = NULL)),
...
O2FRRfMetaNest %>%
  unnest(JVPSII_ETRC_WtTau_O2_predict) %>%
  ggplot() +
  geom_line(aes(x = O2evln_O2ls, y = .fitted), colour = "black", linetype = "dashed") +
 #geom_line(aes(x = O2evln_O2ls, y = .fitted + .se.fit), colour = "grey", linetype = "dashed") +
 #geom_line(aes(x = O2evln_O2ls, y = .fitted - .se.fit), colour = "grey", linetype = "dashed") +
  geom_point(aes(x = O2evln_O2ls, y = JVPSII_ETRC_WtTau),colour = "darkgreen") +
  geom_point(aes(x = O2evln_O2ls, y = .resid), colour = "darkred", size = 0.1) +
  geom_hline(yintercept = 0,linetype = "dashed") +
  facet_grid(cols = vars(Ex_WL)) +
  theme_bw()
...
JVPSII_O2_ModelTerms <- O2FRRfMetaNest %>%
  unnest(c(JVPSII_aLHIIminmax_O2_tidied, JVPSII_ETRC_WtTau_O2_tidied, JVPSII_ETRqpOxbo_O2_tidied), names_sep = "_")
