...
O2Data <- readRDS(file = file.path(DataInO2, paste("FluorO2_", Run, "_O2Data.Rds", sep = ""))) %>%
  mutate(TempCont = Cuvette)

#%>% #add TempCont column for compatibility
...
O2SlopeData <- left_join(x = O2Data, y = FRRfDataLight, by = c("ObsDate", "CultureID" , "Ex_WL", "LR_s", "TempCont"), suffix = c("O2", "FRRf"), keep = FALSE)
...
O2SlopeData <- O2SlopeData %>%
  select(-c("nm445", "nm470","nm505","nm535","nm590","IR"))

#"PIF","LEDSel", "Tau1PQ","Alp2PQ","Tau2PQ","Alp3PQ", "TPQ_PSI", 
```
...
O2SlopeData %>%
  ...
  scale_colour_manual(values = Colours_nm) +
  facet_grid(rows = vars(FileDateTime), cols = vars(CultureID)) +
  # scale_y_continuous(sec.axis = sec_axis(~ . , name = "CultureID", breaks = NULL, labels = NULL)) +
  # scale_x_continuous(sec.axis = sec_axis(~ . , name = "Ex_WL", breaks = NULL, labels = NULL)) +
    theme(strip.text.y = element_text(size = 4, angle=0))

```
Some samples not from MultiCulti; filter out
```{r remove missing data combinations}
O2SlopeDataClean <- O2SlopeData %>%
  filter(as_datetime(FileDateTime) != as_datetime("2022-10-26 10:49:00")) %>%
  filter(CultureID != "JuNa1024",
         CultureID != "MiSa1444",
         CultureID !=  "MiSa1445")
...
O2SlopeDataClean %>%
  ...
  geom_point(aes(x = LR_s, y = ActPARCorr), colour = "black") +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(CultureID), rows = vars(FileDateTime)) +
   # scale_x_continuous(sec.axis = sec_axis(~ . , name = "CultureID", breaks = NULL, labels = NULL)) +
   # scale_y_continuous(sec.axis = sec_axis(~ . , name = "Exp date and time", breaks = NULL, labels = NULL)) +
    theme(strip.text.y = element_text(size = 5, angle=0))
```
...
O2SlopeDataClean %>%
  ...
  scale_colour_manual(values = Colours_nm) +
  facet_grid(rows = vars(Ex_WL), cols = vars(CultureID, FileDateTime)) +
  scale_y_continuous(sec.axis = sec_axis(~ . , name = "CultureID", breaks = NULL, labels = NULL)) +
  scale_x_continuous(sec.axis = sec_axis(~ . , name = "Ex_WL", breaks = NULL, labels = NULL)) +
    theme(strip.text.y = element_text(size = 5, angle=0))
...
O2Slopes <- O2SlopeDataClean %>%
  filter(!is.na(ActPARCorrinter)) %>%
    nest(O2data = -c(FilenameO2, ActPARCorrinter, CultureID, ObsDate, Ex_WL, TempCont))
...
O2Slopes <- O2Slopes %>%
  ...
  group_by(FilenameO2,  ObsDate, CultureID,  Ex_WL, TempCont) %>% 
...
O2FRRfMetaNest <-  O2FRRfMeta %>%
  ...
  nest(data = -c(Ex_WL)) %>%
  mutate(
  JJVPSII_aLHIIminmax_O2 = map(data, possibly(~lm(JVPSII_aLHIIminmax ~ O2evln_O2ls, data = .x), otherwise = NULL)),
  JVPSII_aLHIIminmax_O2_tidied =  map(JJVPSII_aLHIIminmax_O2, possibly(tidy, otherwise = NULL)),
  JVPSII_aLHIIminmax_O2_param = map(JJVPSII_aLHIIminmax_O2, possibly(glance, otherwise = NULL)),
  JVPSII_aLHIIminmax_O2_predict =  map(JJVPSII_aLHIIminmax_O2, possibly(augment, otherwise = NULL)),
  ...
  JVPSII_ETRC_TauAv_O2 = map(data, possibly(~lm(JVPSII_ETRC_TauAv ~ O2evln_O2ls, data = .x), otherwise = NULL)),
  JVPSII_ETRC_TauAv_O2_tidied =  map(JVPSII_ETRC_TauAv_O2, possibly(tidy, otherwise = NULL)),
  JVPSII_ETRC_TauAv_O2_param = map(JVPSII_ETRC_TauAv_O2, possibly(glance, otherwise = NULL)),
  JVPSII_ETRC_TauAv_O2_predict =  map(JVPSII_ETRC_TauAv_O2, possibly(augment, otherwise = NULL)),
...
O2FRRfMetaNest %>%
  unnest(JVPSII_aLHIIminmax_O2_predict) %>%
  ggplot() +
...
O2FRRfMetaNest %>%
  unnest(JVPSII_ETRC_TauAv_O2_predict) %>%
  ggplot() +
  geom_line(aes(x = O2evln_O2ls, y = .fitted), colour = "black", linetype = "dashed") +
 #geom_line(aes(x = O2evln_O2ls, y = .fitted + .se.fit), colour = "grey", linetype = "dashed") +
 #geom_line(aes(x = O2evln_O2ls, y = .fitted - .se.fit), colour = "grey", linetype = "dashed") +
  geom_point(aes(x = O2evln_O2ls, y = JVPSII_ETRC_TauAv),colour = "darkgreen") +
  geom_point(aes(x = O2evln_O2ls, y = .resid), colour = "darkred", size = 0.1) +
  geom_hline(yintercept = 0,linetype = "dashed") +
  # facet_grid(cols = vars(Ex_WL)) +
  theme_bw()
...
```
#XXXXIS THIS WORTH PURSUING FOR SO LITTLE DATA?XXXXX

```{r model terms}
JVPSII_O2_ModelTerms <- O2FRRfMetaNest %>%
  unnest(c(JVPSII_aLHIIminmax_O2_tidied, JVPSII_ETRC_TauAv_O2_tidied, JVPSII_ETRqpOxbo_O2_tidied), names_sep = "_")
