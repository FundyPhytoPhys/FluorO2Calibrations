...
O2FRRfMeta %>%
  ...
  geom_point(aes(x = O2evln_O2lsMIN, y = JVPSII_aLHIIOxbomax, colour = CultureID, shape = O2_Category)) +
   # scale_colour_manual(values = MCColours_nm) +
  facet_grid(cols = vars(Strain), rows = vars(FileDateTime, Par_ue, WL)) +
  theme_bw()
...
Decided to combine all strains in JVPSII calibration; however, after filtering out positive O2_umolLs slopes in the initial dark, only 1 SS120 calibration remains and [PSII] increases with increasing ActPAR. Reconsider including SS120.
...
O2FRRfMetaModels %>%
  unnest(JVPSII_ETRqpOxbo_FoSigmax_m2psii_e_predict, data) %>%
  filter(Ex_WL == 445) |>
  ggplot() +
  geom_line(aes(x = e_Ls, y = .fitted), colour = "black", linetype = "dashed") +
 #geom_line(aes(x = O2evln_O2ls, y = .fitted + .se.fit), colour = "grey", linetype = "dashed") +
