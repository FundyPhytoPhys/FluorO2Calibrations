...
FileID <- "caloxy"
DataIn <- file.path("..", "RawData", "MiSa", "Optode")
...
O2Data <- O2Data %>%
  mutate(Filename = str_remove(string = Filename, pattern = "../RawData/MiSa/Optode/"),
         Filename = str_remove(string = Filename, pattern = "_caloxy")) %>%
  separate(Filename, into = c("FileDateTime", "Project", "CultureID", "Ex_WL"), sep = "([\\/\\_\\.])", remove = FALSE) %>%
...
O2Data %>%
  ...
  facet_grid(rows = vars(Ex_WL, FileDateTime), cols = vars(CultureID)) +
