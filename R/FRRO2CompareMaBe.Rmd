...
ggplot() +
  ...
  geom_point(data = FRRfDataLight, aes(x = LR_s, y = ETRqpOxbo), colour = "black") +
...
O2SlopeData <- left_join(x = O2Data, y = FRRfDataLight, by = c("CultureID", "Ex_WL", "TempCont", "Date", "LR_s"), suffix = c("O2", "FRRf"), keep = FALSE) %>%
...
O2 curves from 2021-12-03 have no matching FRRf data!
```{r O2FRRfprelimplot}
O2SlopeData %>%
  ggplot() +
  geom_point(aes(x = LR_s, y = O2_umolL, colour = as.factor(Date))) +
  geom_jitter(aes(x = LR_s, y = ActPAR, colour = as.factor(Date))) +
  #scale_colour_manual(values = Colours_nm) +
