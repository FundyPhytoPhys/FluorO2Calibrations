...
DataIn <- file.path("..", "RawData", "MiSa", "Solisense", "LRC",  fsep = .Platform$file.sep)
...
SolFitsTrim <- SolFits %>% 
  ...
  select(-c("RFID_User_Data", "Barcode_Data", "PIF",  "Lon", "Lat", "GPS_stat", "Dur-Wat", "PQP_Size", `...33`,`...34`,`...37`,`...38`,`...39`, "Alpha", "Ek", "Pmax" )) %>% # remove superfluous columns
  ...
  #        ) %>%
  mutate(Filename = str_remove(string = Filename, pattern = "../RawData/MiSa/Solisense/LRC/"),
  ...
  # separate(Filename, into = c("Project", "RunDateTime", "CultureID", "Ex_WL"), sep = "([\\/\\_])", remove = FALSE) %>%
    separate(Filename, into = c("Project", "RunDateTime", "CultureID", "Ex_WL", "MeasureO2"), sep = "([\\/\\_])", remove = FALSE) %>%
...
SolFitsTrim <- SolFitsTrim %>%
  group_by(SourceDataFile, StartDateTimeSol, Filename, Project, CultureID, ObsDate, Ex_WL, TempCont) %>%
...
SolFitsTrim2 <- SolFitsTrim %>%
  filter(!is.na(Alp1QA)) %>%  #Some LRC were missing this column
  # filter(CultureID != "JuNa1121",
  #        CultureID != "JuNa1117") %>% 
  group_by(StartDateTimeSol, Filename, Project, ObsDate, CultureID, Ex_WL, TempCont) %>%
...
SolFitsTrim2 %>%
  ...
  scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID, StartDateTimeSol)) +
...
#LRC to PicoDiel
saveRDS(SolFitsTrim2, file.path("..", "..", "PicoDiel", "Data", "ImportedData","ImportedSolisenseData", paste(Run, "LRC_Fits.Rds", sep = "_"), fsep = .Platform$file.sep))
```
