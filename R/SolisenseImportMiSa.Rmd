...
SolFitsTrim <- SolFits %>% 
  ...
  select(-c("RFID_User_Data", "Barcode_Data", "PIF",  "Lon", "Lat", "GPS_stat", "Dur-Wat", "PQP_Size", `...38`, "Alpha", "Ek", "Pmax" )) %>% # remove superfluous columns
...
SolFitsTrim2 %>%
  ...
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x =  JVPSII_aLHIIminmax, y = JVPSII_ETRqpOxbo_FoSigmax_A2psii, colour = as.factor(StartDateTimeSol))) +
  # scale_colour_manual(values = Colours_nm) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
  #coord_fixed(ratio = 1) +
