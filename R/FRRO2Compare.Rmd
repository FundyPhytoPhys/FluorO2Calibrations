...
O2SlopeData <- left_join(x = O2Data, y = FRRfDataLight, by = c("ObsDate", "CultureID" , "Ex_WL","LR_s", "TempCont", "Project", "FileDateTime" = "StartDateTimeSol"), suffix = c("O2", "FRRf"), keep = FALSE) # "FileDateTime" = "StartDateTimeSol" lines up data when replicates are preformed on same day, "ObsDate" does not have time.
...
O2SlopeDataClean <- O2SlopeData %>%
  ...
  filter(!(Ex_WL == 445 & CultureID %in% c("SySl1247")),
         ...
         !(CultureID %in% c("JuNa1024", "MiSa1444", "MiSa1445")))
...
O2Slopes <- O2SlopeDataClean %>%
  filter(!is.na(ActPARCorrinter)) %>%
    nest(O2data = -c(FilenameO2, ActPARCorrinter, CultureID, ObsDate, Ex_WL, TempCont, Project))
...
O2Slopes <- O2Slopes %>%
  ...
  group_by(FilenameO2, Project, ObsDate, CultureID,  Ex_WL, TempCont) %>% 
