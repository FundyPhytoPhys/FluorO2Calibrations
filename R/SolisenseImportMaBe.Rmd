...
#relies upon phiPSII which is not absolute in cyanos
         JVPSII_ETRC_TauAv_FoSig = ETRC_TauAv * Fomin/Sigmax_m2psii,
         JVPSII_ETRqpOxbo_FoSig = ETRqpOxbo * Fomin/Sigmax_m2psii,
         JVPSII_ETRC_TauAv_aLHII_Sig = ETRC_TauAv * aLHIIOxbomax/Sigmax_m2psii,
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = JVPSII_ETRC_TauAv_FoSig)) +
  #geom_point(aes(x = ActPARCorr, y = aLHIIminmax), shape = "cross") +
  #scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL, TempCont), rows = vars(CultureID, ObsDate)) +
  theme_bw()
...
SolFitsTrim2 %>%
  ...
  ggplot() +
  geom_point(aes(x = 
         JVPSII_ETRqpOxbo_FoSig, y = JVPSII_ETRC_TauAv_aLHII_Sig)) +
  #geom_point(aes(x = ActPARCorr, y = aLHIIminmax), shape = "cross") +
  #scale_colour_manual(values = Colours_nm) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
  coord_fixed(ratio = 1) +
