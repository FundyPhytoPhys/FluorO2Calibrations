...
SolFitsTrim <- SolFitsTrim %>%
  group_by(CultureID, ObsDate, Ex_WL, TempCont) %>%
...
SolFitsTrim2 <- SolFitsTrim %>%
  group_by(Filename, Project, ObsDate, CultureID, Ex_WL, TempCont) %>%
  mutate(Fodark = Fo[1],
         Fmdark = Fm[1],
         Sigdark = Sig[1],
...
#relies upon phiPSII which is not absolute in cyanos
         JVPSII_ETRtauav_FoSig = ETRCtauav * Fomin/Sigmax_m2psii,
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = JVPSII_ETRtauav_FoSig)) +
  #geom_point(aes(x = ActPARCorr, y = aLHIIminmax), shape = "cross") +
  #scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL, TempCont), rows = vars(CultureID, ObsDate)) +
  theme_bw()
...
SolFitsTrim2 %>%
  ...
  ggplot() +
  geom_point(aes(x = 
         JVPSII_ETRqpOxbo_FoSig, y = JVPSII_ETRtauav_aLHII_Sig)) +
  #geom_point(aes(x = ActPARCorr, y = aLHIIminmax), shape = "cross") +
  #scale_colour_manual(values = Colours_nm) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
  coord_fixed(ratio = 1) +
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x =  JVPSII_ETRqpOxbo_FoSig, y = JVPSII_aLHIIOxbomax)) +
  #geom_point(aes(x = ActPARCorr, y = aLHIIminmax), shape = "cross") +
  #scale_colour_manual(values = Colours_nm) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
  #coord_fixed(ratio = 1) +
