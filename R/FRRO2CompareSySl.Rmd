...
O2FRRfMetaModels <-  O2FRRfMeta %>%
  mutate(e_Ls = 4 * O2evln_O2ls) |>
  filter(ActPARCorrinter > 0 & ActPARCorrinter < 200) %>%
  nest(data = -c(Ex_WL)) %>%
  mutate(JVPSII_ETRqpOxbo_FoSigmax_m2psii_e = map(data, possibly(~lm(JVPSII_ETRqpOxbo_FoSigmax_m2psii ~  e_Ls, data = .x), otherwise = NULL)),
...
JVPSII_O2_ModelTerms <- O2FRRfMetaModels %>%
...
#JVPSII O2 calibration to BalticPhotoperiod
