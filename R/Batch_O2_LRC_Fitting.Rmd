...
# function with beta photoinhibition
lrc_beta <- function(I, a, b, g, R){(a * (((1 - (b*I)))*I)) - R
}
...
```{r fit models}
#prelim fits
# O2Prelims <- O2FRRfMeta %>%
#   nest(.by = c("ObsDate", "Project",  "Ex_WL", "Strain", "ExpDate":"Temp_c", "O2_Category", "E_days")) %>%
...
#                                data = .x,
#                                start = list(a = (max(.$O2_umolugChls, na.rm = TRUE) - min(.$O2_umolugChls, na.rm = TRUE))/(max(.$ActPARCorrinter, na.rm = TRUE) - min(.$ActPARCorrinter, na.rm = TRUE)),
...
O2Models <- O2FRRfMeta %>%
  filter(!is.na(E_days)) %>%
  nest(.by = c("ObsDate", "Project", "Ex_WL", "Strain", "Par_ue":"Temp_c", "O2_Category", "E_days")) %>%
    mutate(Linear_model = map(data, possibly(~lm(O2_umolugChls ~ ActPARCorrinter, data = .)), NULL)
                        ) %>%
    mutate(Linear_Param = map(Linear_model, tidy)
           ) %>%
  ...
  mutate(LRCbeta_Param = map(LRCbeta_model, tidy),
   ...
   mutate(LRCbeta_model = map(data, possibly(~nlsLM(O2evln_O2ugChls ~ lrc_beta(I = ActPARCorrinter, a, b, R),
                               data = .x,
                               start = list(a = alpha_est,
                                            b = alpha_est/10
                                            R = min(.$O2_umolugChls, na.rm = TRUE))
