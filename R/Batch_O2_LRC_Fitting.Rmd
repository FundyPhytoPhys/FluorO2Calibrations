...
O2Models <- O2FRRfMeta %>%
  nest(.by = c("ObsDate", "FilenameO2", "Project", "CultureID", "Ex_WL", "YYYYMMDDHHMM", "SourceDataFile", "FilenameFRRf", "Strain", "ExpDate":"Run", "E_days")) %>%
    mutate(Linear_model = map(data, ~lm(O2_umolugChls ~ ActPARCorrinter, data = .))
                        )
  ...
  #                        )
  #        ) %>%
  # mutate(LRC_Param = map(LRC_model, possibly(tidy), NULL),
  #        LRC_Predict = map(LRC_model, augment)) %>%

  #        ) %>%
  # mutate(LRCbeta_Param = map(LRCbeta_model, tidy),
  #        LRCbeta_Predict = map(LRCbeta_model, augment))
  # mutate(LRCbeta_model = map(data, possibly(~nlsLM(O2evln_O2ugChls ~ lrc_beta(I = ActPARCorrinter, a, b, g, R),
  #                             data = .x,
  #                             start = list(a = 5E13/100,
  #                                          b = 5E13/1000,
  #                                          g = 5E13/1000,
  #                                          R = min(.$O2_umolugChls, na.rm = TRUE))
  #                        ), NULL)
