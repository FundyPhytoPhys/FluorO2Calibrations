...
```{r fit models}
O2Models <- O2FRRfMeta %>%
  nest(.by = c("ObsDate", "FilenameO2", "Project", "CultureID", "Ex_WL", "YYYYMMDDHHMM", "SourceDataFile", "FilenameFRRf", "Strain", "ExpDate":"Run", "E_days"))
...
%>%
  ...
  mutate(LRCbeta_model = map(data, possibly(~nlsLM(O2_umolugChls ~ lrc_beta(I = ActPARCorr, a, b, g, R),
                              data = .x,
                              start = list(a = 1,
                                           b = 1,
                                           g = 1,
...
%>%
  mutate(LRCbeta_Param = map(LRCbeta_model, tidy),
   ...
   mutate(LRCdR_model = map(data,possibly(~nlsLM(O2_umolugChls ~ lrc_dr(I = ActPARCorr, a, Pmax, R, d),
                              data = .x,
                              start = list(a = 5,
                                           Pmax = 200,
                                           R = 100,
