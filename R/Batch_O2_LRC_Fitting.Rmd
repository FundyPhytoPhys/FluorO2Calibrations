...
It fits and plots models for O2 light response curves, and reports the parameters of the fits.
...
```{r read imported O2 data}
O2FRRfMeta <- readRDS(file.path(O2DataIn, paste(paste(Project, Run, TC, FileID, sep = "_"), ".Rds", sep = "")))

O2Meta <- O2FRRfMeta %>%
  filter(Dark1s == 0) %>%
  select(c(SourceDataFile:ObsTime, Ex_WL:LR_s, Filename, StartDateTimeSol, TempContFRRf, ActPARCorr, O2_umolLs, O2evln_O2ls, O2evln_O2lsMIN, Strain, Par_ue, Photoperiod, Temp_c, O2_Category, WL, LightShape))
...
O2Meta <- left_join(x = O2Meta, y = ChlAv, by = join_by("CultureID", "ObsDate" == "DATE"))
...
O2Meta <- O2Meta %>%
  mutate(O2_umolugChls = O2_umolLs/(meanChl_ugmL * 1000),
    O2evln_umolugChls = O2evln_O2ls/(meanChl_ugmL * 1000))
```
...
Issue with O2FRRfMeta O2 slope models skip ActPARCorr = 0 rows.

# Fit model equations
```{r fit models}
O2Models <- O2Meta %>%
  nest(data = c("ObsTime", "ActPAR", "ActPARCorr",  "LR_s", "O2_umolLs", "O2evln_O2ls", "O2evln_O2lsMIN", "O2_umolugChls", "O2evln_umolugChls"))
       
       ) %>%
  mutate(LRC_model = map(data, possibly(~nlsLM(O2_umolugChls ~ lrc(I = umolphotons_m2_s1, a, Pmax, R),
                              data = .x,
                              start = list(a = 1,
